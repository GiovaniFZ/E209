...
char msg_rx[32];
int pos_msg_rx = 0;
int tamanho_msg_rx = 8;
unsigned int volume = 0, tempo_inf = 0, contador = 0, interv_tempo = 0, cont_aux = 0, erro = 0, data_aux = 0, entrar = 0, pot_aux = 0;
...
int main(void)
{
  ...
  while (1) {
    ...
    if (data_aux >= 100 && data_aux <= 999 && entrar == 1) {
      ...
      tempo_inf = tempo_inf/60;
      // Calculo do fluxo definido (real feito na interrupção)
      fluxo_def = volume/tempo_inf;
      // Calculo da potencia
      pot = (fluxo_def/FLUXO_MAX);
      pot *= 255; // Máximo de intensidade = 255
      pot_aux = (int)pot;
      OCR2A = pot_aux;
    
      data_aux = 0;
      entrar++;
      UART_Transmit("Digite sim para alterar parametros.\n");
