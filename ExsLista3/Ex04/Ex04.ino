...
float DC = 0; // Variavel auxiliar para incrementar 12,5%
unsigned int cont = 0;//conta a cada 100us
short int contTotal = 0; // conta de 1 a 1 segundo

// Interrupção do botão liga/desliga
ISR(PCINT0_vect){
...
ISR(TIMER2_COMPA_vect){
    cont++;
    if(cont >= 100){ //Se passar 1s e não chegar em 8s, ativamos o temporizador
        cont = 0;
        contTotal++;
        DC += 31.87; // 12,5% de 255
        OCR0A = int(DC);
        Serial.print("Cont: ");
        Serial.println(cont);
    }
    if(contTotal > 8) { // Se passou mais que 8 segundos, desativamos o temporizador
        PORTD &= ~MOTOR;
        contTotal = 0;
        cont = 0;
        TCCR2B = 0;
